...
const CertificateCard = ({ cert, aosDelay }) => (
  <article
    ...
    className="bg-black/20 backdrop-blur-sm rounded-2xl p-5 border border-white/10 hover:border-white/25 hover:scale-[1.02] hover:shadow-[0_15px_40px_rgba(0,0,0,0.3)] transition-all duration-500 group"
  >
    <div className="flex items-start gap-4">
      ...
      <div>
        ...
        <p className="text-white/70 text-xs font-medium mt-2">{cert.detail}</p>
        <div className="flex flex-wrap gap-3 mt-4">
          <a href={cert.fileUrl} target="_blank" rel="noopener noreferrer" className="text-xs font-black text-white underline underline-offset-4">
            View certificate
          </a>
          {cert.verifyUrl && (
            <a href={cert.verifyUrl} target="_blank" rel="noopener noreferrer" className="text-xs font-black text-white/70 hover:text-white underline underline-offset-4">
              Verify
            </a>
          )}
        </div>
