...
const Navbar = () => {
  ...
  const hireMeMailto = `mailto:${personalInfo.emails.primary}?subject=Portfolio Inquiry&body=Hello Siddhant,%0D%0A%0D%0AI came across your portfolio and would like to connect.%0D%0A%0D%0ABest Regards,`;

  return (
    <nav className={`fixed top-0 left-0 w-full z-50 transition-all duration-300 ${
      isOpen ? 'bg-[#ff2a2a] py-4' : isScrolled ? 'bg-black/35 backdrop-blur-md py-4' : 'bg-transparent py-6'
    }`}>
      <div className="max-w-7xl mx-auto px-6 md:px-12 flex justify-between items-center">
        <a href="#home" className="text-white text-xl md:text-2xl font-black tracking-tight whitespace-nowrap">
      ...
      <div className={`lg:hidden absolute top-full left-0 w-full transition-all duration-300 overflow-hidden ${
        ...
        <div className="flex flex-col px-6 space-y-4">
          ...
          <a href={hireMeMailto} onClick={() => setIsOpen(false)} className="inline-block px-6 py-3 rounded-full bg-white text-[#ff2a2a] font-black hover:bg-black hover:text-white transition-colors w-full text-center shadow-lg">
            Contact Me
          </a>
        </div>
      </div>
